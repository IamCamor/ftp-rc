...
FRONT="frontend/src"
[ -d "$FRONT" ] || { echo "❌ Не найдена папка $FRONT"; exit 1; }
...
echo "✅ Перезаписан $FRONT/api.ts (добавлены getWeatherFavs/saveWeatherFav)"
echo "Теперь пересобери фронт: npm run build"