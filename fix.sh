...
ROOT="$(pwd)"
COMP="$ROOT/frontend/src/components/BannerSlot.tsx"

if [ ! -d "$ROOT/frontend/src/components" ]; then
  echo "❌ Не найден каталог $ROOT/frontend/src/components — запусти из КОРНЯ репозитория."
  exit 1
fi
...
echo "✅ Обновлён компонент: $COMP"
echo "Готово. Теперь собери фронт:  cd frontend && npm run build"